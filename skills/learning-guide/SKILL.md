...
7. 新学或继续学习时，按 `teachingEntry.sequence` 完成入口导览，再进入诊断、检索或应用；导览本身不算掌握证据。

若用户给出了明确的现有项目目录但尚未注册，在确认目录属于当前任务后使用 `learn project import`；不要仅因名称相似导入目录。

## 教学入口导览

将导览视为教学的先行组织者，不是可选寒暄，也不是完整目录朗读。优先使用 `learn status` 或 `session start` 返回的 `teachingEntry`：

- `project_and_unit_overview`：先做课程总览，再做当前单元导览，然后诊断。
- `project_overview`：说明学习价值、最终能力、核心主线、里程碑/最终证据和第一步；控制在 3–5 分钟，不逐行复述 README。
...
导览必须面向学习者回答“为什么学、现在在哪、这一单元解决什么、怎样算学会”。不要在导览前抛诊断题；不要把导览内容直接写成掌握证据或批量闪卡。
